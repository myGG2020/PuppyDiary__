...
public class DBHelper_kg extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE IF NOT EXISTS kgtest (useridx INTEGER, year INTEGER, month INTEGER, kgs DOUBLE, CONSTRAINT useridx_fk FOREIGN KEY(useridx) REFERENCES user(useridx));");

    }

    public void insert(int useridx, int year, int month, double kgs) {
        // 읽고 쓰기가 가능하게 DB 열기
        SQLiteDatabase db = getWritableDatabase();
        // DB에 입력한 값으로 행 추가
        db.execSQL("UPDATE kgtest SET useridx = " + useridx + " year = " + year + " and month = " + month + " and kgs = " + kgs + ";");
        //db.execSQL("INSERT OR REPLACE INTO kgtest (useridx, pos, year, month, kgs) VALUES(" + useridx + ", " + pos + ", " + year + ", '" + month + "', " +  kgs + ");");

        db.close();
    }

    public double getResult(int useridx, int year, int month) {
        // 읽기가 가능하게 DB 열기
        SQLiteDatabase db = getReadableDatabase();
        double result = 0;

        // DB에 있는 데이터를 쉽게 처리하기 위해 Cursor를 사용
        cursor = db.rawQuery("SELECT kgs from kgtest WHERE useridx = " + useridx + " and month = " + month + " and year = " + year + ";", null);

        if (cursor.getCount() > 0) {
            while (cursor.moveToNext()) {
                result = cursor.getDouble(cursor.getColumnIndex("kgs"));
            }
        }
        cursor.close();
        db.close();
